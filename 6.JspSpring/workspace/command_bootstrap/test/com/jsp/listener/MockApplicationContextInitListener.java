...
public class MockApplicationContextInitListener { // 리스너 테스트라기보다는 코드 테스트.
	
	public void contextInitalized(String beanConfigXml) {
		if(beanConfigXml == null) return;
    	
    	beanConfigXml = beanConfigXml.replace("classpath:", "res/").replace("/", File.separator);
    	// 소스폴더 res에 xml이 있으니까. 거기로 해줘야함
...
//    						System.out.printf("name = %s, ref-value=%s\n", name, ref);
    						...
    						String className = eleBean.getAttribute("class"); // bean에 있는 elBean(name)의 class를 가져옴.
