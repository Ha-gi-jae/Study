...
public class FileUploadController {
	...
	public ModelAndView uploadByMultipartFile(@RequestParam(value = "title", defaultValue = "제목없음")String title, @RequestParam("file")MultipartFile multi, HttpServletRequest request, HttpServletResponse response, ModelAndView mnv) throws Exception{
	...
	public ModelAndView uploadByMultipartHttpServletRequest(MultipartHttpServletRequest request, HttpServletResponse response, ModelAndView mnv) throws Exception{
	...
	private void fileUpload(MultipartFile multi, String title, HttpServletRequest request, HttpServletResponse response, ModelAndView mnv) throws Exception {
		...
		// 파일 저장폴더 설정
		String uploadPath = request.getSession().getServletContext().getRealPath("resources/upload");// 배포폴더에다가 저장.
