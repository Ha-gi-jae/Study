...
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {
    ...
    private Map<String, String[]> parameterMap;											// 폼필드(파라미터) 데이터를 담기 위한 맵
    					// 여러개 값 날라오는 것 까지 처리 할 수 있게하려고 배열로 선언함.
    ...
    private void parsing(HttpServletRequest request,
        ...
        if (ServletFileUpload.isMultipartContent(request)) {
            ...
            ServletFileUpload servletFileUpload = new ServletFileUpload(factory); // 업로드 시점에 세팅 
    ...
    public String[] getParameterValues(String name) {
        if (this.isMultipart) // 멀티파트일때 파싱해서 담아논 데이터를 꺼내오고
            return (String[])parameterMap.get(name);
        else				// 멀티파트 아니면 기존 데이터 가져옴.
