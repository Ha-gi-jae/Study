...
public class T01_FileTest {
	public static void main(String[] args) throws IOException {
		...
		System.out.println("파일 여부 : " + file.isFile()); // 파일인지 아닌지 확인해준다. boolean으로 return
		...
		System.out.println(file3.getName() + "의 용량 크기 : " + file3.length() + "bytes");
															// length() 크기 가져옴. 바이트단위로.
		// 3. new File(String parent, String child)
		File file4 = new File("./d_Other/test/..", "test.txt"); 
		System.out.println("절대 경로 : " + file4.getAbsolutePath());
		// 절대 경로 : 내가 현재 어디에 위치하든지 상관없이 그 위치를 나타낸다. 드라이브부터 시작한다.
		System.out.println("경로 : " + file4.getPath());	// 작성한 경로.
		System.out.println("표준 경로 : " + file4.getCanonicalPath()); // 의미 하는 경로를 찾아감. 
		System.out.println("------------------------------------");
		// 프로젝트 경로 => 현재 경로 
